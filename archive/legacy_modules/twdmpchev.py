...
def run_model(X_train, y_train, X_test, y_test, beta, alpha):
    ...
    # 使用向量化计算距离矩阵，切比雪夫距离
    dis_arr = pairwise_distances(X_train, metric='chebyshev')  # 修改为切比雪夫距离
    ...
    def process_test_sample(i, t):
        # 预计算测试样本 t 到所有训练样本的切比雪夫距离
        distances = np.max(np.abs(X_train - t), axis=1)  # 修改为切比雪夫距离
        
        # 使用集合推导式过滤符合条件的 cliques_label
